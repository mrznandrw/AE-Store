...
const UpdateProperties = (props) => {
    ...
    const append = () => {
        setProperties([...properties, {id: null, title: '', description: '', unique: uuid(), append: true}])
    }
    const remove = (unique) => {
                ...
                <Row key={item.unique} className="mb-2" style={{display: item.remove ? 'none': 'flex'}}>
                    <Col>
                        <Form.Control
                            ...
                            value={item.title}
                            onChange={e => change('title', e.target.value, item.unique)}
                    ...
                    <Col>
                        <Form.Control
                            ...
                            onChange={e => change('description', e.target.value, item.unique)}
