...
import { fetchOneProduct, updateProduct, fetchCategories, fetchBrands } from '../../http/productAPI.js'
import { useState, useEffect } from 'react'
import uuid from 'react-uuid'
import UpdateProperties from './UpdateProperties.js'
import { createProperty, updateProperty, deleteProperty } from '../../http/productAPI.js'

const defaultValue = {name: '', price: '', category: '', brand: '',  quantity: '', description: '', title: ''}
const defaultValid = {name: null, price: null, category: null, brand: null, quantity: null, description: null, title: null}
...
const isValid = (value) => {
    ...
    for (let key in value) {
        ...
        if (key === 'quantity') result.quantity = pattern.test(value.quantity)
        if (key === 'description') result.description = value.name.trim() !== ''
        if (key === 'title') result.title = value.name.trim() !== ''
    }
    return result
}
...
const updateProperties = async (properties, productId) => {
    for (const prop of properties) {
        const empty = prop.title.trim() === '' || prop.description.trim() === ''
...
const UpdateProduct = (props) => {
            ...
            fetchOneProduct(id)
                .then(
                    data => {
                        ...
                        setProperties(data.info.map(item => {
    ...
    const handleSubmit = async (event) => {
        ...
        if (correct.name && correct.price && correct.category && correct.brand && correct.quantity && correct.description && correct.title) {
